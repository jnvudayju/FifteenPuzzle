...
public class FifteenPuzzle {
	...
	public static void main(String[] args) {
		...
		String isSwingInput = "y";
		String answer;
		FifteenPuzzle p;

		System.out.print("\nDo you want to take input from swing window	Y//N: ");
		isSwingInput = s.nextLine();

		if (isSwingInput.charAt(0) == 'y') {
			INPUT_FROM_FILE = false;
			TARGET_FROM_FILE = false;
		} else {
		...
		// Swing Input
		if (isSwingInput.charAt(0) == 'y') {
			FifteenPuzzleSwingInput swingInput = new FifteenPuzzleSwingInput(p);

			try {
				synchronized (swingInput) {
					swingInput.wait();
				}
			...
			for (int i = 0; i < PUZZLE_SIZE; i++) {
				for (int j = 0; j < PUZZLE_SIZE; j++) {
					p.tiles[i][j] = swingInput.input[i][j];
					p.target[i][j] = swingInput.target[i][j];
				}
			...
			if (answer.charAt(0) == 'y') {
				...
				p.shuffle(SHUFFLE_ITERATION); // Number of shuffles is critical -- large numbers (100+) and 4x4 puzzle
												// is
				// hard even for A*.
			}
		}
		...
		final List<FifteenPuzzle> solution;
